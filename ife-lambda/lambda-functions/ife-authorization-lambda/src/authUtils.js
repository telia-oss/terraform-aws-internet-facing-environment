const isResourceAllowed = (resource, scopes) => {
    let allowedScopes = [];
    if (resource != undefined && resource.length > 1 && scopes != undefined && scopes.length > 0) {
        scopes.split(' ').forEach((sc) => {
            allowedScopes.push(sc.split('/')[1]);
        });
    } else {
        return false;
    }

    return allowedScopes.includes(resource.replace(/\//g, ''));
}
...
module.exports = { isResourceAllowed, AuthPolicy }
